...
export async function startGenAiSession({
	...
	function handleModelTurn(message: LiveServerMessage) {
		...
		if (message.serverContent?.inputTranscription?.text) {
			...
			genaiLogger.log("[user transcription]", transcript);
		...
		if (message.serverContent?.outputTranscription?.text) {
			...
			genaiLogger.log("[assistant transcription]", transcript);
			onAssistantTranscription?.(transcript);
		}
