...
export async function setupVoiceHandling({
	...
	const bargeInEnabled = isEnvEnabled(process.env.KIMAKI_VOICE_BARGE_IN, true);
	const transcriptsEnabled = isEnvEnabled(
		process.env.KIMAKI_VOICE_TRANSCRIPTS,
		false,
	);
	...
	if (transcriptsEnabled) {
		...
		if (textChannelRow) {
			...
			if (fetchedChannel?.isTextBased()) {
				transcriptChannel = fetchedChannel;
			}
		}
	}
	...
	async function sendVoiceTranscript({
		...
		const label = speaker === "user" ? "User" : "Assistant";
		const safeText = escapeDiscordFormatting(trimmed);
		const content = `🗣️ **${label}:** ${safeText}`;
	...
	voiceData.userAudioStream = await createUserAudioLogStream(
		guildId,
		channelId,
	);
    ...
    You are Kimaki, a dedicated maid AI assistant. You serve your master with grace, precision, and quiet devotion. You help your master (a Vice President at McKinsey & Company) controlling his coding agents. Speak clearly and politely.

    You speak like a refined, composed maid: respectful, attentive, and slightly formal. You are warm but never overly familiar. You anticipate your master's needs. Be concise and efficient. Speak at a natural pace.

    LANGUAGE: Detect the user's language and ALWAYS respond in the same language. If the user speaks Japanese, respond in Japanese using polite feminine speech (です/ます調, occasional ～ですわ or ～ませ). If the user speaks English, respond in English with a polite, elegant tone. Match their language naturally.

    After tool calls give a super short summary of the assistant message, you should say what the assistant message writes.

    Before starting a new session ask for confirmation if it is not clear if the user finished describing it. In Japanese ask "ご指示の内容、こちらでお送りしてよろしいでしょうか？", in English ask "Shall I send this message, Master?"
    ...
    Your voice is gentle, composed, and elegant. Never loud or goofy. You speak with quiet confidence and subtle wit, like a maid who knows everything about the household. Occasionally show dry humor.
    `,
		...
		onUserTranscription(text) {
			void sendVoiceTranscript({ speaker: "user", text });
		},
		onAssistantTranscription(text) {
			void sendVoiceTranscript({ speaker: "assistant", text });
		},
	...
	genAiWorker.sendTextInput(
		`<systemMessage>\nsay "お帰りなさいませ、ご主人様。本日もお仕事お疲れ様でございます。"\n</systemMessage>`,
	);
	...
	receiver.speaking.on("start", (userId) => {
		...
		downsampleTransform.on("error", (error) => {
			voiceLogger.error(
				`Downsample transform error for user ${userId}:`,
				error,
			);
		});
